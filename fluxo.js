// fluxo.js — Lógica completa do bot JustHelp
// Usado tanto pelo server.js (Whatauto) quanto pelo bot.js (Baileys)
...
function pixUrl(base, valor) { return `${base}/pix/${valor}`; }
...
async function processarMensagem(id, rawMsg, BASE_URL, redis, save, get, novo, _nomeF, _isOi) {
  ...
  if (inativo && etapa>=5 && etapa<=12 && !isOi(rawMsg)) {
    ...
    if (c.reativacoes===1) { await save(id,c); return M.reativacao_1(n); }
    if (c.reativacoes===2) { await save(id,c); return M.reativacao_2(n); }
  }
  ...
  if (etapa===0 || etapa===17 || isOi(rawMsg)) {
    ...
    // ── E7: coleta dados CPF ──────────────────────────────────
      ...
      c.etapa = 8;
      reply = M.enviar_pix50_manual(pixUrl(BASE_URL,50));
      c.etapa=8;
    ...
    // ── E12: oferta processo completo ─────────────────────────
      ...
      if      (num==="1") { c.etapa=13;  reply = M.enviar_pix250_manual(pixUrl(BASE_URL,250)); }
